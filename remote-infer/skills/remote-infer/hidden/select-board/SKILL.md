...
This helper satisfies the `select-board` step in `workflows/remote-infer.yaml`. Prefer using `core/board_client.py` when running from this repository so selection logic stays centralized:

```bash
python3 remote-infer/core/board_client.py [--chip AX650N]
```
