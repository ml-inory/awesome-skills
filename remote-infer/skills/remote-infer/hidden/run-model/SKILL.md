...
This helper satisfies the `run-model` step in `workflows/remote-infer.yaml`. Prefer using `core/infer.py` when running from this repository:

```bash
python3 remote-infer/core/infer.py <MODEL_PATH> --host <BOARD_IP> [--input <INPUT_FILE>] [--port 18500]
```
